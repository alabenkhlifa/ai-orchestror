...
A user with separate GitHub and passwordless hosted identities can enter one stable passwordless account when identity ownership is proven, while preserving every hosted project and preventing unsafe automatic matches, partial merges, silent overwrites, or involuntary re-linking.
...
4. A valid unambiguous match starts a non-mutating preflight of both hosted identities and their complete project sets.
5. Project-name or repository conflicts pause the merge for explicit recovery choices and a fresh preflight.
6. A conflict-free merge commits atomically into the existing passwordless personal workspace, preserves every project, revokes absorbed-workspace worker pairings, and reduces the absorbed workspace to the approved minimal record.
7. The user may later unlink GitHub only after fresh passwordless proof and explicit confirmation.
8. A later GitHub sign-in cannot undo that unlink automatically; explicit re-linking requires proof of both sign-in methods, confirmation, preflight, and repository revalidation.
...
- Idempotent automatic identity linking into the stable passwordless workspace.
...
- Automatic linking uses only the single GitHub email marked both primary and verified.
...
- Missing, unverified, non-matching, or ambiguous primary results skip automatic linking without disclosing another account.
...
- A normalized form matching multiple hosted identities stops automatic linking without account disclosure.
- Automatic linking is idempotent and preserves the passwordless user and personal-workspace identities.
...
- Automatic merge attempts and results are disclosed and auditable under the approved GDPR data contract.
...
- Given either address contains non-ASCII characters or an `xn--` label, when automatic matching is considered, then no Unicode or IDNA transformation or automatic link occurs.
- Given normalization maps to multiple identities, when matching ends, then no merge or account disclosure occurs.
- Given matching identities have no project conflict, when merge commits, then the passwordless identity and workspace survive and every hosted project is preserved exactly once.
...
- Can explicit re-link proceed when GitHub returns no verified primary email, provided both sign-in methods are freshly proven?
