...
GitHub and passwordless hosted access can create separate stable identities and personal workspaces for the same person. Email-based automatic linking is security-sensitive because provider email semantics, aliases, reassignment, normalization, project conflicts, worker credentials, and retained merge evidence can cause account takeover or data loss.

This design uses conservative automatic matching and a separate explicit two-proof re-link path after user unlink.
...
4. Preflight the complete resulting project set and all workspace ownership changes without mutation.
5. Collect explicit conflict choices and rerun the full preflight.
6. Commit identity attachment, project moves, worker revocation, merge record creation, and audit atomically.
7. Support fresh-passwordless-proof unlinking and persist a minimal policy that suppresses future automatic re-link.
8. Support explicit re-link by proving both sign-in methods, confirming the accounts, rerunning preflight, and revalidating repositories.
...
- Automatic-match interface: enforce ASCII eligibility, approved canonicalization, collision checks, and account-neutral failure.
...
- Merge-commit interface: attach GitHub, move every hosted project, revoke source pairings, create the minimal merge record, and audit atomically.
...
- Consequence: Explicit re-link may join different verified emails because control of both identities replaces email equality, while automatic linking rules stay unchanged.

## Risks

- Incorrect normalization can merge different people. Fail closed, version provider rules, and test collisions and ambiguity.
...
- Which provider domains and account types enter the launch normalization registry?
- How are registry review, versioning, deployment, rollback, and rule removal governed?
- Which lawful basis and retention are approved for the merge record and unlink policy?
- Which support and user recovery process handles an incorrect merge?
- Is a verified primary GitHub email required for explicit re-link after both methods are proven?
- Which transaction, locking, idempotency, notification, audit, and credential-revocation mechanisms implement the contracts?
- Which privacy, security, integration, concurrency, and browser proof forms the verification gate?
