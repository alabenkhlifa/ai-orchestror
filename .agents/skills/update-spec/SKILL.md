...
10. Set status from the resulting state. Move requirements to `Draft` or tasks to `Blocked` when decisions remain, and remove `Verified` whenever existing proof no longer covers the changed behavior.
...
12. Report the changed decisions, newly exposed questions, invalidated or deferred work, status changes, and whether implementation can resume.
